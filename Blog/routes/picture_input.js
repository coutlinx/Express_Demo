...
router.get("/", function(req, res, next) {
    ...
    if (req.session.MD != undefined && req.session.compile) {
        ...
        res.render("admin/pictures-input", {
            MD: req.session.MD,
...
router.post("/", (req, res) => {
    ...
    let ID;
    config.db.query(
        "select article_id from essay where article_md =?", [req.body.content],
        (err, results, fild) => {
            if (err != null) {
                console.log(err);
            ...
            if (!ex) {
                config.db.query(
                    "select use_id from user where use_name = ?", [req.session.user.name],
                    (err, results, fil) => {
                        if (err != null) {
                            console.log(err);
                        } else {
