     ...
     if(req.session.usename==undefined){
        ...
        data.db.query("select max(id) as id from user_friends UNION ALL SELECT user_friends.use_friends_id as fid FROM user_friends WHERE use_id=?",[m[0].use_id],(err,results)=>{
         if(err!=null){
            ...
            datas=results;
        data.db.query("insert into user_friends(id,use_friends_name,use_add_time,use_id,use_friends_id,use_friends_email) value(?,?,?,?,?,?)",[datas[0].id+1,req.body.blogname,nowTime,m[0].use_id,datas[0].datas[0].fid+1,req.body.blogemail],(err,results)=>{
          if(err!=null){
              console.log(err)
          }else{
            console.log(results);
