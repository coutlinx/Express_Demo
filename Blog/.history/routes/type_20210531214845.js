...
router.get('/', function(req, res, next) {
  ...
  // data.db.query("select sort_name from classify ORDER BY article_id LIMIT 0,5 ",(err,results,fild)=>{
  //   if(err !=null){
  //     console.log(err)
  //   }else{
...
router.post('/Class',(req,res)=>{
  config.db.query("select * from essay where article_classify = ? ORDER BY article_id LIMIT 0,5 ",[req.body.Type],(err,results,fild)=>{
    if(err !=null){
      console.log(err)
    }else{
