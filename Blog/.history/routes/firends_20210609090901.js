  ...
  if(req.session.usename==undefined){
    ...
    db.db.query("select * from user_friends as u where u.use_id=(select user.use_id from user where use_name=?)",[req.session.usename],(err,results)=>{
      if(err !=null){
        ...
        datail=results
        console.log(datail);
        db.db.query("SELECT * FROM essay INNER JOIN user_friends ON user_friends.use_friends_id=essay.use_id WHERE essay.use_id=? ORDER BY article_id LIMIT 0,5",[datail[0].use_friends_id],(err,results)=>{
          if(err !=null){
            console.log(err)
          }else{
              datas=results;
