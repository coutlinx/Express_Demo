    ...
    config.db.query(
      ...
      (err, results, fil) => {
        if (err != null) {
          ...
          let id = results[0].use_id;
          config.db.query(
            "update  essay set use_id =? ,article_title = ?,article_content =?,article_date =?,article_photo =?,article_md =?,article_html =?,article_classify =?,article_status =?,article_recommend =?,article_type_creat =?,article_recommend_status =?,article_admire =?,article_reprint =?,article_discuss =?",
            [
              id,
              req.body.title,
              req.body.description,
